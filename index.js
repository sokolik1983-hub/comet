    ...
    var $pTw7$export$default =
        ...
        function () {
            function THREERoot(params) {
                ...
                this.renderer = new THREE.WebGLRenderer({ //инициализируем WebGL
                    antialias: antialias,
                    alpha: alpha,
                    canvas: canvas
                });
                ...
                //     _this.resize(); //Подстраивает размер звезды под размер экрана
                // }); // tick / update / render
...
// ASSET: shaders/shooting-star.frag
    ...
    var $a87C$var$PER_MOUSE = 800; //Распыление от звезды
    var $a87C$var$COUNT = $a87C$var$PER_MOUSE * 400; ////непонятно
    ...
    //Sokol1
    var $a87C$var$uniformData = { //Данные по характеристикам звезды
        size: {
            value: 0.05, //размер распыления чапстичек звезды
            range: [0, 1]
        },
        minSize: {
            value: 1,
        ...
        speed: { //Скорость звезды
            value: 0.012,
            range: [0, 0.05]
        },
        fadeSpeed: {
            value: 1.1, //Скорость исчезновения следа
            range: [1, 2]
        },
        shortRangeFadeSpeed: {
            value: 1.3,
    ...
    var $a87C$export$default =
        /*#__PURE__*/
        function () {
            function ShootingStar() { //Функция вызова звезды
                var _this = this;
                ...
                console.log('1111111111111root', root)
                this.setSize();
                ...
                var uniforms = {
                    ...
                    pixelRatio: {
                        value: root.renderer.getPixelRatio() //Какая то стандартная функция у рендерера
                    },
                    timestamp: {
                        value: 0
                    }
                ...
                console.log('$a87C$var$uniformData', $a87C$var$uniformData);
                ...
                ;
                const string = "precision highp float;\nprecision highp int;\n#define GLSLIFY 1\nattribute vec3 position;\nuniform mat4 modelViewMatrix;\nuniform mat4 projectionMatrix;\n\nattribute vec4 mouse;\nattribute vec2 aFront;\nattribute float random;\n\n// uniform float uProgress;\nuniform vec2 resolution;\nuniform float pixelRatio;\nuniform float timestamp;\n\nuniform float size;\nuniform float minSize;\n// uniform float delay;\nuniform float speed;\nuniform float far;\nuniform float spread;\nuniform float maxSpread;\nuniform float maxZ;\nuniform float maxDiff;\nuniform float diffPow;\n\nvarying float vProgress;\nvarying float vRandom;\nvarying float vDiff;\nvarying float vSpreadLength;\nvarying float vPositionZ;\n\nfloat cubicOut(float t) {\n  float f = t - 1.0;\n  return f * f * f + 1.0;\n}\n\n// #pragma glslify: cubicBezier = require(../../modules/cubicBezier.glsl)\n\nconst float PI = 3.1415926;\nconst float PI2 = PI * 2.;\n\nvec2 cartesianToPolar (vec2 p) {\n  return vec2((atan(p.y, p.x) + PI) / PI2, length(p));\n}\n\nvec2 polarToCartesian (vec2 p) {\n  float r = p.x * PI2 - PI;\n  float l = p.y;\n  return vec2(cos(r) * l, sin(r) * l);\n}\n\nvoid main () {\n  // float progress = max(uProgress - random * delay, 0.);\n  float progress = clamp((timestamp - mouse.z) * speed, 0., 1.);\n  progress *= step(0., mouse.x);\n\n  float startX = mouse.x - resolution.x / 2.;\n  float startY = mouse.y - resolution.y / 2.;\n  vec3 startPosition = vec3(startX, startY, random);\n\n  float diff = clamp(mouse.w / maxDiff, 0., 1.);\n  diff = pow(diff, diffPow);\n\n  vec3 cPosition = position * 2. - 1.;\n\n  float radian = cPosition.x * PI2 - PI;\n  vec2 xySpread = vec2(cos(radian), sin(radian)) * spread * mix(1., maxSpread, diff) * cPosition.y;\n\n  vec3 endPosition = startPosition;\n  endPosition.xy += xySpread;\n  endPosition.xy -= aFront * far * random;\n  endPosition.z += cPosition.z * maxZ * (pixelRatio > 1. ? 1.2 : 1.);\n\n  float positionProgress = cubicOut(progress * random);\n  // float positionProgress = cubicBezier(.29, .16, .3, 1., progress);\n  vec3 currentPosition = mix(startPosition, endPosition, positionProgress);\n\n  vProgress = progress;\n  vRandom = random;\n  vDiff = diff;\n  vSpreadLength = cPosition.y;\n  vPositionZ = position.z;\n\n  gl_Position = projectionMatrix * modelViewMatrix * vec4(currentPosition, 1.);\n  gl_PointSize = max(currentPosition.z * size * diff * pixelRatio, minSize * (pixelRatio > 1. ? 1.3 : 1.));\n}\n"

                const string2 = "precision highp float;\nprecision highp int;\n#define GLSLIFY 1\n\n// uniform float uProgress;\nuniform float fadeSpeed;\nuniform float shortRangeFadeSpeed;\nuniform float minFlashingSpeed;\nuniform float blur;\n\nvarying float vProgress;\nvarying float vRandom;\nvarying float vDiff;\nvarying float vSpreadLength;\nvarying float vPositionZ;\n\nhighp float random(vec2 co)\n{\n    highp float a = 12.9898;\n    highp float b = 78.233;\n    highp float c = 43758.5453;\n    highp float dt= dot(co.xy ,vec2(a,b));\n    highp float sn= mod(dt,3.14);\n    return fract(sin(sn) * c);\n}\n\nfloat quadraticIn(float t) {\n  return t * t;\n}\n\n#ifndef HALF_PI\n#define HALF_PI 1.5707963267948966\n#endif\n\nfloat sineOut(float t) {\n  return sin(t * HALF_PI);\n}\n\nconst vec3 baseColor = vec3(170., 133., 88.) / 255.;\n\nvoid main(){\n\tvec2 p = gl_PointCoord * 2. - 1.;\n\tfloat len = length(p);\n\n  float cRandom = random(vec2(vProgress * mix(minFlashingSpeed, 1., vRandom)));\n  cRandom = mix(0.3, 2., cRandom);\n\n  float cBlur = blur * mix(1., 0.3, vPositionZ);\n\tfloat shape = smoothstep(1. - cBlur, 1. + cBlur, (1. - cBlur) / len);\n  shape *= mix(0.5, 1., vRandom);\n\n  if (shape == 0.) discard;\n\n  float darkness = mix(0.1, 1., vPositionZ);\n\n  float alphaProgress = vProgress * fadeSpeed * mix(2.5, 1., pow(vDiff, 0.6));\n  alphaProgress *= mix(shortRangeFadeSpeed, 1., sineOut(vSpreadLength) * quadraticIn(vDiff));\n  float alpha = 1. - min(alphaProgress, 1.);\n  alpha *= cRandom * vDiff;\n\n\tgl_FragColor = vec4(baseColor * darkness * cRandom, shape * alpha);\n}\n"

                var material = this.material = new THREE.RawShaderMaterial({
                    uniforms: uniforms, //++
                    vertexShader: string,//++
                    fragmentShader: string2,//++
                    transparent: true,//++
                    depthTest: false,//++
                ...
                // mesh.visible = false;
                root.add(this.mesh); // root.initPostProcessing([
...
// ASSET: shaders/text.frag
    ...
    function $Focm$var$_defineProperties(target, props) {
        console.log('отрисовка первой звезды')
        for (var i = 0; i < props.length; i++) {
            var descriptor = props[i];
            descriptor.enumerable = descriptor.enumerable || false;
            descriptor.configurable = true;
    ...
    function $Focm$var$_createClass(Constructor, protoProps, staticProps) {
        console.log('создаём класс $Focm$var$_createClass', Constructor)
        //Вторым аргументом сюда идёт отрисовка первой звезды
    ...
    var $Focm$var$CAMERA_Z = 5000;  //непонятно
    var $Focm$var$MAX_CAMERA_Z = 5000; //непонятно
    var $Focm$var$FIRST_DURATION = 1080; //скорость первой звезды
    var $Focm$var$DELAY = 300; //Через сколько вся красота запустится после загрузки окна
    var $Focm$var$data = {
        play: {
            value: null
        }
    ...
    var $Focm$var$WebGL =
        ...
        function () {
            function WebGL(_ref) {
                ...
                var controller = new $IDtB$export$default({ //инициализация контроллера
                    closed: true
                });
                ...
                console.log('$mrfc$export$default.root', $mrfc$export$default.root)
                var root = this.root = $mrfc$export$default.root = new $pTw7$export$default({ //инициализация руут ?
                    isDev: true,
                    container: container,
            ...
            , {
                ...
                value: function start() {
                    ...
                    $Zk$export$animate(function (progress) {
                        ...
                        //     setTimeout(function () {
                        //         _this2.textStart(); //вызывает отрисовку теста посередине и после эффект на движение мышки
                        //     }, 300);
                        // }
                    });
                }
            ...
            //
            //         $Zk$export$animate(function (progress) {
            //             console.log('333333333333333333', progress);
            //             _this3.shootingStar.draw({
            //                 clientX: progress,
            //                 clientY: 0 //y где отрисовывается вторая звезда посередине экрана с текстом
            //             });
            //
